...
Генерация:
/generatemessage, /genmsg, /gm [normal|absurd|chaos] - сгенерировать Markov-сообщение из текущего чата; без режима используется режим чата
/demotivatorgeneration, /demgen, /d [normal|absurd|chaos] - сгенерировать демотиватор из текста чата и сохранённого фото или импортированной картинки
/generatebugurt, /genbug, /b - сгенерировать legacy-бугурт из текущего чата; режим чата не используется

Картинки, цитаты, голос:
/createdemotivator, /crdem, /cd <верх>|<низ> - демотиватор из фото в сообщении или reply
/createquote, /cq, /q - цитата из reply-сообщения и аватарки автора
/voiceover, /v <text> - озвучить текст или reply-сообщение

Обучение и импорт:
/learn_history - только админы; импортировать экспорт истории из приложенного документа или reply на документ
/readtread2ch, /rt2ch <thread_or_board_url> - только админы; импортировать тред или доску 2ch

Настройки и сервис:
/set_mode <normal|absurd|chaos> - только админы; установить режим генерации по умолчанию
/stats, /s - статистика текущего чата

Автообучение: в группах и супергруппах бот запоминает обычный текст, подписи и фото; команды, начинающиеся с /, не обучают корпус.
